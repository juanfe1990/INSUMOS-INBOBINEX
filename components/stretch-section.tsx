...
import { ArrowRight, CheckCircle2, Layers, Palette, Ruler, Scale, Sparkles } from 'lucide-react';
...
interface StretchOption {
    ...
    title: string;
    tagline: string;
    defaultWeight: number; // en gramos
    suggestedWeights: number[]; // en gramos
    description: string;
    applications: string[];
    specs: {
        gauge: string;
...
const STRETCH_OPTIONS: StretchOption[] = [
    {
        ...
        defaultWeight: 500,
        suggestedWeights: [300, 500, 800, 1000, 1500],
        description:
    ...
    },
    {
        ...
        defaultWeight: 1000,
        suggestedWeights: [500, 1000, 1500, 2000, 2500],
        description:
    ...
    },
    {
        ...
        defaultWeight: 2000,
        suggestedWeights: [1000, 1500, 2000, 2500, 3000, 3300],
        description:
    ...
    },
    {
        ...
        defaultWeight: 2500,
        suggestedWeights: [1500, 2000, 2500, 3000, 3300],
        description:
    ...
    },
    {
        ...
        defaultWeight: 3300,
        suggestedWeights: [2000, 2500, 3000, 3300],
        description:
            'Diseñado para cobertura total rápida en operaciones industriales intensivas y exportación. Ofrece el mayor rendimiento por metro envuelto y máxima contención.',
        applications: [
...
export function StretchSection() {
    ...
    const [color, setColor] = useState<'transparente' | 'negro'>('transparente');
    const [weight, setWeight] = useState<number>(2000); // gramos
    ...
    const activeImage = color === 'negro'
        ? activeOption.image.replace('.svg', '-black.svg')
        : activeOption.image;

    // Estimar metros aproximados según peso en gramos y ancho (promedio 1kg = 1000g en 50cm cal 50 = ~130m)
    const estimatedMeters = Math.round(((weight / 1000) * 6500) / activeOption.widthCm);

    const colorName = color === 'negro' ? 'Negro' : 'Transparente';

    // Mensaje dinámico para WhatsApp
    const whatsappMessage = `Hola Inbobinex, me interesa cotizar el Papel Stretch ${colorName} en medida de ${activeOption.widthCm} cm (${activeOption.title}) con un peso personalizado de ${weight} g. ¿Me podrían brindar información de precio y disponibilidad?`;
    const whatsappUrl = `https://wa.me/${WHATSAPP_PHONE}?text=${encodeURIComponent(whatsappMessage)}`;

    const jsonLd = {
        '@context': 'https://schema.org',
        '@type': 'Product',
        'name': `${activeOption.title} - ${colorName}`,
        'image': `https://inbobinex.com${activeImage}`,
            ...
            <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                ...
                <div className="text-center">
                    ...
                    <p className="mx-auto mt-4 max-w-2xl text-base text-[#617064] sm:text-lg">
                        Selecciona el ancho del rollo, el color (Transparente o Negro) y ajusta el peso en gramos (hasta 3.300 g) según las necesidades de tu empresa.
                    </p>
                </div>
                    ...
                    {STRETCH_OPTIONS.map((opt) => {
                        ...
                        return (
                            <button
                                ...
                                className={`flex items-center gap-2 rounded-xl px-4 py-3 text-sm font-bold transition-all duration-200 ${isActive
                ...
                <div className="mt-12 grid gap-8 rounded-3xl border border-[#C8CDC9]/70 bg-white p-6 shadow-xl lg:grid-cols-12 lg:p-10">
                    ...
                    <div className="flex flex-col items-center justify-between rounded-2xl bg-gradient-to-b from-white via-[#F7FAF8] to-[#EBF0EC] p-6 border border-[#C8CDC9]/60 shadow-sm lg:col-span-5">
                        <div className="w-full flex items-center justify-between gap-2">
                            <span className="rounded-full bg-[#2E4040] px-3.5 py-1 text-xs font-semibold text-white shadow-sm">
                                Ancho: {activeOption.widthCm} cm
                            </span>
                            <span className="rounded-full bg-[#3F8346]/10 border border-[#3F8346]/20 px-3.5 py-1 text-xs font-bold text-[#3F8346]">
                                Peso: {weight} g
                            </span>
                        </div>

                        {/* Roll Visual Studio Container */}
                        <div className="relative my-6 aspect-square w-full max-w-[340px] overflow-hidden rounded-2xl shadow-lg border border-[#C8CDC9]/40 bg-white transition-all duration-300 hover:shadow-xl">
                            <Image
                                key={`${activeOption.id}-${color}`}
                                src={activeImage}
                                alt={`${activeOption.title} - ${colorName}`}
                                fill
                        ...
                        <div className="grid w-full grid-cols-2 gap-3 pt-2">
                            ...
                            <div className="rounded-xl border border-[#C8CDC9]/50 bg-white p-3 text-center shadow-sm">
                                ...
                                <span className="font-heading text-sm font-bold text-[#2E4040] truncate block capitalize">
                                    {colorName}
                                </span>
                            </div>
                        </div>
                    ...
                    <div className="flex flex-col justify-between lg:col-span-7">
                        <div>
                            ...
                            <div className="mt-6 rounded-2xl bg-[#F4F7F5]/80 p-5 border border-[#C8CDC9]/50">
                                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                                    <label className="flex items-center gap-2 font-bold text-[#2E4040]">
                                        <Palette className="h-5 w-5 text-[#3F8346]" />
                                        <span>Color de Stretch:</span>
                                    </label>
                                    <div className="flex items-center gap-2">
                                        <button
                                            type="button"
                                            onClick={() => setColor('transparente')}
                                            className={`flex items-center gap-2 rounded-xl px-3.5 py-2 text-xs font-bold transition-all ${color === 'transparente'
                                                ? 'bg-[#3F8346] text-white shadow-md ring-2 ring-[#3F8346]'
                                                : 'bg-white text-[#2E4040] border border-[#C8CDC9] hover:border-[#3F8346]'
                                                }`}
                                        >
                                            <span className="h-3.5 w-3.5 rounded-full border border-white/60 bg-emerald-100 shadow-inner" />
                                            Transparente
                                        </button>
                                        <button
                                            type="button"
                                            onClick={() => setColor('negro')}
                                            className={`flex items-center gap-2 rounded-xl px-3.5 py-2 text-xs font-bold transition-all ${color === 'negro'
                                                ? 'bg-[#1A1D1A] text-white shadow-md ring-2 ring-[#1A1D1A]'
                                                : 'bg-white text-[#2E4040] border border-[#C8CDC9] hover:border-black'
                                                }`}
                                        >
                                            <span className="h-3.5 w-3.5 rounded-full bg-slate-900 border border-slate-700 shadow-inner" />
                                            Stretch Negro
                                        </button>
                                    </div>
                                </div>
                            ...
                            <div className="mt-4 rounded-2xl bg-[#F4F7F5]/80 p-5 border border-[#C8CDC9]/50">
                                <div className="flex items-center justify-between">
                                    ...
                                    <span className="font-heading text-xl font-extrabold text-[#3F8346]">
                                        {weight} <span className="text-sm font-normal text-[#2E4040]">g</span>
                                    </span>
                                </div>
                                ...
                                <div className="mt-4 flex flex-wrap items-center gap-2">
                                    ...
                                    {activeOption.suggestedWeights.map((w) => (
                                        <button
                                            key={w}
                                            onClick={() => setWeight(w)}
                                            className={`rounded-lg px-2.5 py-1 text-xs font-bold transition-colors ${weight === w
                        ...
                        <div className="mt-8 pt-4 border-t border-[#C8CDC9]/40">
                            ...
                            >
                                ...
                                <span>Cotizar {activeOption.widthCm}cm ({weight} g) - {colorName} por WhatsApp</span>
